...
| `PrdCheckpoint` | Validate the expected PRD revision, enforce one legal gate transition, patch only canonical lifecycle truth, and re-read an attested result. Assertions and merge guards fail closed on drift. Callers never patch PRD lifecycle state directly. |
