...
Read `references/prd-checkpoint.md`. Resolve the canonical local vault root, then invoke the packaged absolute command `"${CLAUDE_PLUGIN_ROOT}/scripts/prd_checkpoint.py" --vault-root <root>` through Python with one JSON transition request—including current `blockers` or `none`—on stdin. Use its `--guard source-mutation` before PRD-owned repository edits and `--guard yield` before settlement. Use `--validate` for read-only structural validation and `--migrate` for revision-safe v0.3.1 migration. Never patch PRD lifecycle fields through the Obsidian connector. Fill the request's `repository` field with the output of `"${CLAUDE_PLUGIN_ROOT}/scripts/repo_fingerprint.py" <repository-root>`—not a freeform description—so it stays byte-identical to what the packaged `Stop` hook independently recomputes when checking it against `Current checkpoint`.
...
The packaged plugin hooks (`hooks/hooks.json`) enforce the same two guards deterministically, without relying on the model to remember to call them. Every successful `activate`, `assert-active`, or other transition arms an on-disk record for this repository; `hooks/pre_tool_use.py` then runs `--guard source-mutation` before every `Edit`, `Write`, `MultiEdit`, `NotebookEdit`, or `Bash` call and blocks it on refusal, and `hooks/stop.py` runs `--guard yield` before the turn ends and blocks a stale, nonterminal yield. Both hooks no-op immediately when this repository has never been armed, so standalone Lean work and unrelated projects are unaffected. Pausing or completing every gate clears the record; the next `activate`/`assert-active` re-arms it.
